...
#define L2_FILTER_SIZE 5*5*48
#define L3_FILTER_SIZE 3*3*256
#define L4_FILTER_SIZE 3*3*192
#define L5_FILTER_SIZE 3*3*192
...
void read_input(float *Layer1_Neurons_CPU){
	read_parameter("data1/input_ts.txt", Layer1_Neurons_CPU);
}
...
void read_bias(float *Layer1_bias_CPU,float *Layer2_bias_CPU,float *Layer3_bias_CPU,float *Layer4_bias_CPU,float *Layer5_bias_CPU,float *Layer6_bias_CPU,float *Layer7_bias_CPU,float *Layer8_bias_CPU){
	read_parameter("data1/bias1.txt", Layer1_bias_CPU);
	read_parameter("data1/bias2.txt", Layer2_bias_CPU);
	read_parameter("data1/bias3.txt", Layer3_bias_CPU);
	read_parameter("data1/bias4.txt", Layer4_bias_CPU);
	read_parameter("data1/bias5.txt", Layer5_bias_CPU);
	read_parameter("data1/bias6.txt", Layer6_bias_CPU);
	read_parameter("data1/bias7.txt", Layer7_bias_CPU);
	read_parameter("data1/bias8.txt", Layer8_bias_CPU);
}
...
void read_weights(float *Layer1_Weights_CPU,float *Layer2_Weights_CPU,float *Layer3_Weights_CPU,float *Layer4_Weights_CPU,float *Layer5_Weights_CPU,float *Layer6_Weights_CPU,float *Layer7_Weights_CPU,float *Layer8_Weights_CPU){
	read_parameter("data1/conv1.txt", Layer1_Weights_CPU);
	read_parameter("data1/conv2.txt", Layer2_Weights_CPU);
	read_parameter("data1/conv3.txt", Layer3_Weights_CPU);
	read_parameter("data1/conv4.txt", Layer4_Weights_CPU);
	read_parameter("data1/conv5.txt", Layer5_Weights_CPU);
	read_parameter("data1/fc6.txt", Layer6_Weights_CPU);
	read_parameter("data1/fc7.txt", Layer7_Weights_CPU);
	read_parameter("data1/fc8.txt", Layer8_Weights_CPU);
}
...
void NeuralNetwork(){
	...
	//* First Layer *//
	cudaMalloc((void**) &Layer1_bias, L1_OUT * sizeof(float)); //96
	cudaMalloc((void**) &Layer1_Neurons, INPUT_SIZE * sizeof(float)); //227*227*3
	cudaMalloc((void**) &Layer1_Weights, (L1_OUT*L1_FILTER_SIZE) * sizeof(float)); //96*11*11*3 = 34848
	cudaMalloc((void**) &Layer1_norm, (L1_OUT*L1_FM_SIZE) * sizeof(float)); //96*55*55
	...
	dim3 Conv11_Block(96,1,1);
	dim3 Conv11_Thread(32,32);
	first_conv<<<Conv11_Block,Conv11_Thread>>>(Layer1_bias,Layer1_Neurons,Layer1_Weights,Layer1_norm,227,3,55,55,4,11,0,0);
	dim3 Conv12_Block(96,1,1);
	dim3 Conv12_Thread(32,23);
	first_conv<<<Conv12_Block,Conv12_Thread>>>(Layer1_bias,Layer1_Neurons,Layer1_Weights,Layer1_norm,227,3,55,55,4,11,0,32);
	dim3 Conv13_Block(96,1,1);
	dim3 Conv13_Thread(23,32);
	first_conv<<<Conv13_Block,Conv13_Thread>>>(Layer1_bias,Layer1_Neurons,Layer1_Weights,Layer1_norm,227,3,55,55,4,11,32,0);
	dim3 Conv14_Block(96,1,1);
	dim3 Conv14_Thread(23,23);
	first_conv<<<Conv14_Block,Conv14_Thread>>>(Layer1_bias,Layer1_Neurons,Layer1_Weights,Layer1_norm,227,3,55,55,4,11,32,32);

	cudaMalloc((void**) &Layer1_pool, (L1_OUT*L1_FM_SIZE) * sizeof(float)); //96*55*55

	/* Normalization of First Layer */
	dim3 Norm11_Block(96,1,1);
	dim3 Norm11_Thread(32,32);
	norm<<<Norm11_Block,Norm11_Thread>>>(Layer1_norm, Layer1_pool,  0.0001, 0.75, 5, 96, 55, 55, 0, 0);
	dim3 Norm12_Block(96,1,1);
	dim3 Norm12_Thread(32,23);
	norm<<<Norm12_Block,Norm12_Thread>>>(Layer1_norm, Layer1_pool,  0.0001, 0.75, 5, 96, 55, 55, 0, 32);
	dim3 Norm13_Block(96,1,1);
	dim3 Norm13_Thread(23,32);
	norm<<<Norm13_Block,Norm13_Thread>>>(Layer1_norm, Layer1_pool,  0.0001, 0.75, 5, 96, 55, 55, 32, 0);
	dim3 Norm14_Block(96,1,1);
	dim3 Norm14_Thread(23,23);
	norm<<<Norm14_Block,Norm14_Thread>>>(Layer1_norm, Layer1_pool,  0.0001, 0.75, 5, 96, 55, 55, 32, 32);

	cudaMalloc((void**) &Layer2_Neurons, (L1_OUT*L2_FM_SIZE) * sizeof(float)); //96*27*27

	/* Maxpooling of First Layer */
	dim3 Pool1_Block(96,1,1);
	dim3 Pool1_Thread(27,27);
	maxpool<<<Pool1_Block,Pool1_Thread>>>(Layer1_pool, Layer2_Neurons, 96, 27, 27, 2, 3, 55, 55, 0, 0);

	//* Second Layer *//
	cudaMalloc((void**) &Layer2_bias, L2_OUT * sizeof(float)); //256
	cudaMalloc((void**) &Layer2_Weights, (L2_OUT*L2_FILTER_SIZE) * sizeof(float)); //256*5*5*48 = 307200
	cudaMalloc((void**) &Layer2_norm, (L2_OUT*L2_FM_SIZE) * sizeof(float)); //256*27*27
	...
	// GPU 1
	conv<<<Layer2_Block,Layer2_Thread>>>(Layer2_bias, Layer2_Neurons, Layer2_Weights, Layer2_norm, 27, 27, 1, 2, 5, 48, 0, 0, 0, 0);
	// GPU 2
	conv<<<Layer2_Block,Layer2_Thread>>>(Layer2_bias, Layer2_Neurons, Layer2_Weights, Layer2_norm, 27, 27, 1, 2, 5, 48, 128, 1, 0, 0);

	cudaMalloc((void**) &Layer2_pool, (L2_OUT*L2_FM_SIZE) * sizeof(float)); //256*27*27

	/* Normalization of Second Layer */
	dim3 Norm2_Block(256,1,1);
	dim3 Norm2_Thread(27,27);
	norm<<<Norm2_Block,Norm2_Thread>>>(Layer2_norm, Layer2_pool, 0.0001, 0.75, 5, 256, 27, 27, 0, 0);
	
	cudaMalloc((void**) &Layer3_Neurons, (L2_OUT*L3_FM_SIZE) * sizeof(float)); //256*13*13

	/* Maxpooling of Second Layer */
	dim3 Pool2_Block(256,1,1);
	dim3 Pool2_Thread(13,13);
	maxpool<<<Pool2_Block,Pool2_Thread>>>(Layer2_pool, Layer3_Neurons, 256, 13, 13, 2, 3, 27, 27, 0, 0);

	//* Third Layer *//
	cudaMalloc((void**) &Layer3_bias, L3_OUT * sizeof(float)); //384
	cudaMalloc((void**) &Layer3_Weights, (L3_OUT*L3_FILTER_SIZE) * sizeof(float)); //384*3*3*256 = 884736
	...
	conv<<<Layer3_Block,Layer3_Thread>>>(Layer3_bias, Layer3_Neurons, Layer3_Weights, Layer4_Neurons, 13, 13, 1, 1, 3, 256, 0, 0, 0, 0);

	//* Fourth Layer *//
	cudaMalloc((void**) &Layer4_bias, L4_OUT * sizeof(float)); //384
	cudaMalloc((void**) &Layer4_Weights, (L4_OUT*L4_FILTER_SIZE) * sizeof(float)); //384*3*3*192 = 663552
	cudaMalloc((void**) &Layer5_Neurons, (L4_OUT*L5_FM_SIZE) * sizeof(float)); //384*13*13
	...
	// GPU 1
	conv<<<Layer4_Block,Layer4_Thread>>>(Layer4_bias, Layer4_Neurons, Layer4_Weights, Layer5_Neurons, 13, 13, 1, 1, 3, 192, 0, 0, 0, 0);
	// GPU 2
	conv<<<Layer4_Block,Layer4_Thread>>>(Layer4_bias, Layer4_Neurons, Layer4_Weights, Layer5_Neurons, 13, 13, 1, 1, 3, 192, 192, 1, 0, 0);

	//* Fifth Layer *//
	cudaMalloc((void**) &Layer5_bias, L5_OUT * sizeof(float)); //256
	cudaMalloc((void**) &Layer5_Weights, (L5_OUT*L5_FILTER_SIZE) * sizeof(float)); //256*3*3*192 = 442368
	...
	// GPU 1
	conv<<<Layer5_Block,Layer5_Thread>>>(Layer5_bias, Layer5_Neurons, Layer5_Weights, Layer5_pool, 13, 13, 1, 1, 3, 192, 0, 0, 0, 0);
	// GPU 2
	conv<<<Layer5_Block,Layer5_Thread>>>(Layer5_bias, Layer5_Neurons, Layer5_Weights, Layer5_pool, 13, 13, 1, 1, 3, 192, 128, 1, 0, 0);
	...
	maxpool<<<Pool3_Block,Pool3_Thread>>>(Layer5_pool, Layer6_Neurons, 256, 6, 6, 2, 3, 13, 13, 0, 0);
	...
	fc<<<Layer6_Block,Layer6_Thread>>>(Layer6_bias, Layer6_Neurons, Layer6_Weights, Layer7_Neurons, 4096, (6*6*256), true);
	...
	fc<<<Layer7_Block,Layer7_Thread>>>(Layer7_bias, Layer7_Neurons, Layer7_Weights, Layer8_Neurons, 4096, 4096, true);
	...
	fc<<<Layer8_Block,Layer8_Thread>>>(Layer8_bias, Layer8_Neurons, Layer8_Weights, Result_Neurons, 1000, 4096, false);
