...
# Execute '/usr/local/cuda/bin/nvcc -Xcompiler -fPIC -shared -o priorwork_models2.so priorwork_models2.cu' first.
...
def alex_inference(stopping, starttime_list, endtime_list, stop):
    pid = os.getpid()
    print('Alexnet Inference PID : {0}'.format(pid))

    dll = ctypes.CDLL('./priorwork_models2.so', mode=ctypes.RTLD_GLOBAL)
...
def res_inference(stopping, starttime_list, endtime_list, stop):
    pid = os.getpid()
...
def vgg_inference(stopping, starttime_list, endtime_list, stop):
    pid = os.getpid()
