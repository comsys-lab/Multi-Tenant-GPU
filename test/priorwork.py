...
# Execute '/usr/local/cuda/bin/nvcc -Xcompiler -fPIC -shared -o priorwork_models.so priorwork_models.cu' first.
...
def aras_inference(count, user_request, random_model, total, stopping, producer_status, finisher):
    pid = os.getpid()
    print('Inference PID : {0}'.format(pid))

    dll = ctypes.CDLL('./priorwork_models.so', mode=ctypes.RTLD_GLOBAL)
    ...
    while True:
        if(user_request.empty()):
            if(producer_status.value == 0):        #user_request는 비어있지만 request creator는 아직 작동중
                ...
                break
        elif(user_request.qsize() < 3):
            continue
        else:
            file = open("result_aras.txt","a")
            ...
            if request_count > (total - 1):
                ...
                alex_arrival_time = Queue()
                res_arrival_time = Queue()
                vgg_arrival_time = Queue()
                ...
                for i in range(3):
                    if random_model[0] == 'alexnet':
                        del random_model[0]
                        alex_arrival_time.put(user_request.get())
                        alex_num += 1                       
                    elif random_model[0] == 'resnet18':
                        del random_model[0]
                        res_arrival_time.put(user_request.get())
                        res_num += 1
                    elif random_model[0] == 'vgg16':
                        del random_model[0]
                        vgg_arrival_time.put(user_request.get())
                        vgg_num += 1
                    else:
                        continue
                ...
                for i in range(alex_num):
                    queueing_delay = alex_start_time - alex_arrival_time.get()
                    alex_queueing_delay.put(queueing_delay)
                    alex_first_conv(A_L1_b,A_L1_N,A_L1_w,A_L1_norm)
                res_start_time = time.time()
                for i in range(res_num):
                    queueing_delay = res_start_time - res_arrival_time.get()
                    res_queueing_delay.put(queueing_delay)
                    res_first_conv(R_L1_N,R_L1_w,R_L1_bn,R_L1_pool,R_L1_M,R_L1_V,R_L1_G,R_L1_B)
                vgg_start_time = time.time()
                for i in range(vgg_num):
                    queueing_delay = vgg_start_time - vgg_arrival_time.get()
...
if __name__ == '__main__':
    ...
    children.append(Process(target = aras_inference, args=(count, user_request, random_model, total, stopping, producer_status, finisher)))
